...
def main():
    ...
    .main {{
        background-image: url(
            "https://avatars.dzeninfra.ru/get-zen_doc/1718877/\
            pub_61cacc9bd1e72b6a321017e8_62335a98f781ab27c5f97f55/scale_1200");
...
def StyleButton(widget_label, font_color='white',
        ...
        <script>
            ...
            for (var i = 0; i < elements.length; ++i) {{
                if (elements[i].innerText == '{widget_label}') {{
                    ...
                    // Делаем блочным элементом для использования margin:auto
...
def home_page():
    ...
    .underline {
        ...
        /* Отрицательное значение отступа для приближения к заголовку */
        margin-top: -30px;
    }
    .subtitle {
    ...
    st.markdown("<p class='subtitle'>\
                Мы определяем название достопримечательностей Москвы<br>\
                с помощью нейросети</p>", unsafe_allow_html=True)
    st.markdown("<p class='subtitle2'>\
                Вы можете загрузить свою фотографию и узнать\
                название достопримечательности</p>",
                unsafe_allow_html=True)
    # Создание кнопки "Попробовать" с использованием пользовательского CSS
    StyleButton('Попробовать', width='250px',
                height='70px', margin_top='200px')
...
def second_page():
    ...
    if st.button("Распознать", key="recognition_button"):
        # Вывод результата
        st.markdown("<p class='subtitle3'>\
                    Распознанная достопримечательность:</p>",
                    unsafe_allow_html=True)
